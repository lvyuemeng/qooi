...
def test_review_blocks_side_when_action_surface_gate_fails() -> None:
    config = PotentialConfig(max_staleness_hours=24)
    ranked = pl.DataFrame(
        {
            "symbol": ["RE-USDT-SWAP"],
    ...
    action_surface = pl.DataFrame(
        {
            "action_side": ["down", "down", "up"],
            "actionability": ["trade_candidate", "no_action", "trade_candidate"],
            "calibrated_side_margin": [-0.1, -0.1, 0.1],
            "false_direction_int": [1, 0, 0],
        }
    )

    decisions = review_decisions(ranked, freshness, {}, config, action_surface)

    assert decisions[0].symbol == "RE-USDT-SWAP"
    assert decisions[0].action == "watch"
    assert decisions[0].reason == "side gate blocked: market-state only; do not promote short"


def test_review_prefers_primary_up_when_blocked_down_scores_higher() -> None:
    config = PotentialConfig(max_staleness_hours=24)
    ranked = pl.DataFrame(
        {
            "symbol": ["RE-USDT-SWAP", "RE-USDT-SWAP"],
            ...
            "direction": ["down", "up"],
            "outcome_horizon": [24, 24],
            "rank_score": [60.0, 10.0],
            "support_count": [4378.0, 1200.0],
            "tail_lift": [44.2, 8.0],
            "utility_proxy": [3.1, 2.0],
            "source_freshness": ["fresh", "fresh"],
            "required_missing_source_count": [0, 0],
            "required_stale_source_count": [0, 0],
        }
