...
from qooi.research.data import apply_signal_debug_filters, mtf_history_requests
...
def test_strategy_registry_builds_baselines():
    ...
    selection = build_strategies(BENCHMARK_GROUPS["baselines"], args)
...
def test_strategy_registry_builds_structural_event_reversal():
    strategy = strategy_from_name("structure_event_reversal_v1", _args())

    assert strategy.name == "structure_event_reversal_v1"


def test_strategy_registry_builds_structural_event_trend_aligned():
    strategy = strategy_from_name("structure_event_trend_aligned_v1", _args())

    assert strategy.name == "structure_event_trend_aligned_v1"


def test_strategy_registry_builds_mtf_confirm_structural_variant():
    strategy = strategy_from_name("structure_event_trend_aligned_mtf_confirm_v1", _args())

    assert strategy.name == "structure_event_trend_aligned_mtf_confirm_v1"


def test_mtf_history_requests_include_base_and_context_targets():
    config = resolve_config(
        _args(strategy="structure_event_trend_aligned_mtf_confirm_v1", days=730, min_bars=12000)
    )
    pair = PAIRS[0]

    requests = mtf_history_requests(
        (pair,),
        _args(strategy="structure_event_trend_aligned_mtf_confirm_v1"),
        config,
        refresh=True,
    )
    ...
    assert (pair.asset.symbol, "15m") in by_bar
    assert by_bar[(pair.asset.symbol, "15m")].min_bars == 70_080
...
def test_strategy_registry_builds_no_volume_structural_variants():
    reversal = strategy_from_name("structure_event_reversal_no_vol_v1", _args())
    trend_aligned = strategy_from_name("structure_event_trend_aligned_no_vol_v1", _args())
...
def test_selected_strategy_names_supports_comma_list_and_benchmark_group():
    assert selected_strategy_names(
        _args(strategies="ema_trend_baseline,rsi_macd_trend")
    ) == ("ema_trend_baseline", "rsi_macd_trend")
    assert selected_strategy_names(_args(benchmark=True)) == BENCHMARK_GROUPS["baselines"]
...
def test_research_profile_keeps_default_basket_stacking_cap():
    config = resolve_config(_args(profile="research"))
...
def test_report_status_flags_yield_divergence():
    ...
    gates = resolve_config(_args()).risk_gates
...
def test_report_status_flags_execution_infeasible_min_contract_runs():
    ...
    gates = resolve_config(_args(profile="safe", min_pf=0.0, min_trades=0)).risk_gates
    ...
    signal_frame = pl.DataFrame(
        {
            ...
            "market_stage": ["warmup", "wide_range", "trend_continuation", "transition"]
            * 3,
            "market_stage_reason": [
...
def test_signal_debug_filters_suppress_side_without_mutating_frame():
    ...
    config = resolve_config(_args(long_only=True))

    filtered = apply_signal_debug_filters(frame, config)
...
def test_signal_debug_filters_include_and_exclude_signal_ids():
    ...
    included = apply_signal_debug_filters(frame, resolve_config(_args(include_signal_id="a,c")))
    excluded = apply_signal_debug_filters(frame, resolve_config(_args(exclude_signal_id="b")))
...
def test_signal_debug_filter_metadata_is_recorded():
    config = resolve_config(_args(short_only=True, exclude_signal_id="long_rule"))
