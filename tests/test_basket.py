...
from qooi.core.basket import Basket, BasketAction, BasketManager, Position
...
def test_basket_properties():
    b = Basket(basket_id="test", symbol="ETH", strategy="m", side="buy", state="idle")
...
def test_basket_action_defaults():
    a = BasketAction(basket_id="x", action="enter", reason="signal")
    assert a.side == ""
    assert a.px == 0.0
