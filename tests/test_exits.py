...
from qooi.core.exits import ExitConfig, TrailTracker, evaluate
...
def test_stop_hit_long():
    ...
    a = evaluate(b, bar_close=94.0, bar_high=100, bar_low=94, atr=5.0, trail=t, config=cfg)
...
def test_stop_not_hit_if_target_first():
    ...
    a = evaluate(b, bar_close=94.0, bar_high=110, bar_low=94, atr=5.0, trail=t, config=cfg)
    assert a is None or a.reason != "stop"
...
def test_target_activates_trail():
    ...
    a = evaluate(b, bar_close=106.0, bar_high=106, bar_low=105, atr=5.0, trail=t, config=cfg)
...
def test_trailing_stop_after_target():
    ...
    a = evaluate(b, bar_close=95.0, bar_high=100, bar_low=95, atr=5.0, trail=t, config=cfg)
    assert a is not None
    assert a.reason == "trailing_stop"
...
def test_time_stop():
    ...
    a = evaluate(b, bar_close=101.0, bar_high=101, bar_low=101, atr=5.0, trail=t, config=cfg)
    assert a is not None
...
def test_breakeven_stop():
    ...
    a = evaluate(b, bar_close=99.0, bar_high=99, bar_low=99, atr=5.0, trail=t, config=cfg)
    assert a is not None
    assert a.reason == "breakeven"
...
def test_no_exit_when_in_range():
    ...
    a = evaluate(b, bar_close=101.0, bar_high=101, bar_low=100, atr=5.0, trail=t, config=cfg)
    assert a is None
