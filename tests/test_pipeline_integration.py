...
def test_signal_entry_creates_basket():
    ...
    actions = process_bar(df, baskets, pair, signal_src=1.0)
...
def test_basket_does_not_duplicate_on_same_signal():
    ...
    actions2 = process_bar(df, baskets, pair, signal_src=1.0)
    assert len(actions2) == 0


def test_signal_flip_exits_basket():
    """Layer 2: signal=1→-1 produces EXIT via signal_flip."""
    pair = _pair()
    df = _df([100.0, 101.0])
    baskets: list[Basket] = []

    process_bar(df, baskets, pair, signal_src=1.0)
    assert baskets[0].is_active

    actions = process_bar(df, baskets, pair, signal_src=-1.0)
    assert len(actions) == 1
    assert actions[0].action == ActionKind.EXIT
    assert actions[0].reason == ExitReason.SIGNAL_FLIP.value
    assert baskets[0].is_idle


def test_signal_zero_exits_basket():
    """Layer 2: signal=1→0 produces EXIT via signal_zero."""
    pair = _pair()
    df = _df([100.0, 101.0])
    baskets: list[Basket] = []
    ...
    actions = process_bar(df, baskets, pair, signal_src=0.0)
    assert len(actions) == 1
    assert actions[0].action == ActionKind.EXIT
...
def test_bars_in_pos_accumulates():
    ...
    process_bar(_df(prices[:2]), baskets, pair, signal_src=1.0)
...
def test_time_stop_fires():
    ...
    process_bar(df, baskets, pair, exit_cfg=cfg, signal_src=1.0)
    assert baskets[0].is_active
...
def test_grid_recovery_activates():
    ...
    baskets: list[Basket] = []
    rec = RecoveryConfig(strategy=RecoveryKind.GRID, zone_atr=1.0, multiplier=2.0, max_levels=3)

    # Bar 1: entry at close=100.0
    process_bar(_df([100.0, 100.0], atr=3.0), baskets, pair, recovery_cfg=rec, signal_src=1.0)
    assert baskets[0].is_active
    assert baskets[0].entry_px == 100.0

    # Bar 2: close drops to 97.0, loss=-3%, ATR=3, zone_atr=1 → grid level 1
    actions = process_bar(
        _df([100.0, 97.0], atr=3.0), baskets, pair, recovery_cfg=rec, signal_src=1.0
    )
...
def test_martingale_produces_exit_and_enter():
    ...
    pair = _pair(capital=500.0)
    baskets: list[Basket] = []
    rec = RecoveryConfig(strategy=RecoveryKind.MARTINGALE, zone_atr=1.0, max_levels=3)
...
def test_recovery_preempts_exit_same_bar():
    ...
    pair = _pair(capital=500.0)
    baskets: list[Basket] = []
    rec = RecoveryConfig(strategy=RecoveryKind.GRID, zone_atr=1.0, multiplier=2.0)
    cfg = ExitConfig(stop_mult=0.1, target_mult=100.0, max_bars=0)

    process_bar(
        _df([100.0, 100.0], atr=3.0), baskets, pair, recovery_cfg=rec, exit_cfg=cfg, signal_src=1.0
    )
    actions = process_bar(
        _df([100.0, 97.0], atr=3.0), baskets, pair, recovery_cfg=rec, exit_cfg=cfg, signal_src=1.0
    )
...
def test_multiple_baskets_per_strategy():
    """Layer 2: BasketManager limits 1 basket per symbol+strategy."""
    pair = _pair()
    df = _df([100.0, 101.0])
    baskets: list[Basket] = []
    ...
    # Same signal on same symbol should not create second basket
    actions = process_bar(df, baskets, pair, signal_src=1.0)
