...
def kline_path_rows(rows: pl.DataFrame, ngram_length: int) -> pl.DataFrame:
    ...
    state_changed = (
        pl.col("state_key") != pl.col("state_key").shift(1).over("symbol")
    ).fill_null(True)
    event_changed = (
        pl.col("context_event") != pl.col("context_event").shift(1).over("symbol")
    ).fill_null(True)
    return rows.sort("symbol", "timestamp").with_columns(
        pl.col("state_key")
        .str.split("|")
    ...
    for horizon in sorted({int(horizon) for horizon in horizons if int(horizon) > 0}):
        ...
        frames.append(
            base.with_columns(
                ...
                (pl.col("terminal_regime_state") != pl.col("regime_state")).alias(
                    "regime_changed"
                ),
