"""Tailtree extreme-event policy and path labels."""
...
class TailEventPolicy:
    ...
    def reference_frame(self, outcomes: pl.DataFrame) -> pl.DataFrame:
        ...
        reference = depths.group_by("outcome_horizon", "side").agg(
            ...
            pl.col("tail_depth_pct").quantile(quantile, interpolation="nearest").alias(
                "quantile_depth_pct"
            ),
        ...
        if self.extreme.method == "fixed_pct":
            ...
            threshold = pl.max_horizontal(
                pl.lit(material_floor), pl.col("quantile_depth_pct")
            )
