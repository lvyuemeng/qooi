...
from qooi.dynamic import states, vq_rssm
from qooi.dynamic.contracts import CodeSequence, SequenceDataset, WindowDataset
...
class PreparedStateDiscovery:
    ...
    def model_spec(self) -> vq_rssm.VqRssmSpec:
        ...
        future_contrast = self.config.future_contrast
        ...
        return vq_rssm.VqRssmSpec(
            ...
            normalize_codebook=config.normalize,
            reset_dead_codes=config.reset_dead_codes,
            reset_interval=config.reset_interval,
            reset_threshold=config.reset_threshold,
            reset_fraction=config.reset_fraction,
            reset_warmup_epochs=config.reset_warmup_epochs,
            reset_candidate_similarity_max=config.reset_candidate_similarity_max,
            ...
            future_min_len=objective.future_config.min_len,
            future_max_len=objective.future_config.max_len,
            future_samples=objective.future_config.samples,
            future_dim=objective.future_config.dim,
            future_temperature=future_contrast.temperature
            if future_contrast.temperature is not None
            else objective.future_config.temperature,
            future_source=objective.future_config.source,
            future_length_policy=objective.future_config.length_policy,
            future_warmup_epochs=objective.future_config.warmup_epochs,
            future_standard_weight=future_contrast.standard_weight,
            future_similarity_weight=future_contrast.future_similarity_weight,
            future_similarity_top_k=future_contrast.future_similarity_top_k,
            future_similarity_mse_weight=future_contrast.future_similarity_mse_weight,
            future_similarity_cosine_weight=future_contrast.future_similarity_cosine_weight,
            future_similarity_max_distance=future_contrast.future_similarity_max_distance,
            future_detrend=objective.future_config.detrend,
            future_detrend_half_life=objective.future_config.detrend_half_life,
            temporal_consistency_weight=temporal_consistency.weight,
            temporal_consistency_temperature=temporal_consistency.temperature,
        )
    ...
    def training_schedule(self) -> tuple[vq_rssm.VqRssmScheduleStage, ...]:
        return tuple(
            vq_rssm.VqRssmScheduleStage(
                name=stage.name,
                start_epoch=stage.start_epoch,
                epochs=stage.epochs,
                reconstruct_weight=stage.reconstruct,
                future_weight=stage.future,
                diversity_weight=stage.diversity_weight,
                reset_fraction=stage.reset_fraction,
                reset_dead_codes=stage.reset_dead_codes,
                freeze_encoder_blocks=stage.freeze_encoder_blocks,
                lr=stage.lr,
            )
            for stage in self.config.schedule.stages
        )
    ...
    def predict_states(
        ...
        if self.config.postprocess.min_state_duration > 0:
            codes = states.filter_short_state_runs(
                codes,
                symbols=provenance.symbols,
                splits=provenance.splits,
                min_duration=self.config.postprocess.min_state_duration,
            )
